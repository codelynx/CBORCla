...
public struct IANATags {
    ...
    public static let registry: [UInt64: TagDefinition] = [
        ...
        // COSE Tags (RFC 9052)
        16: TagDefinition(name: "coseEncrypt0", description: "COSE Single Recipient Encrypted Data", dataItem: .array()),
        ...
        27: TagDefinition(name: "genericObject", description: "Generic object (class, data)", dataItem: .array(elements: 2)),
        ...
        38: TagDefinition(name: "languageTaggedString", description: "Language-tagged string", dataItem: .array(elements: 2)),

        // Identifier
        39: TagDefinition(name: "identifier", description: "Identifier", dataItem: .any),

        // Multi-dimensional Arrays (40-41)
        40: TagDefinition(name: "multiDimArrayRowMajor", description: "Multi-dimensional array, row-major", dataItem: .array()),
        ...
        256: TagDefinition(name: "stringReferencesNamespace", description: "String references namespace", dataItem: .any),
        ...
        // RAINS
        15309736: TagDefinition(name: "rainsMessage", description: "RAINS message", dataItem: .array()),

        // Intel FPGA
        4294967296: TagDefinition(name: "intelFPGA", description: "Intel FPGA SPDM Manifest", dataItem: .any),

        // Reserved Invalid
        65535: TagDefinition(name: "invalid16", description: "Invalid (Reserved)", dataItem: .invalid),
        4294967295: TagDefinition(name: "invalid32", description: "Invalid (Reserved)", dataItem: .invalid),
        18446744073709551615: TagDefinition(name: "invalid64", description: "Invalid (Reserved)", dataItem: .invalid)
    ]
...
public enum DataItemRequirement {
    ...
    public func validate(_ value: CBORValue) -> Bool {
        ...
        case .byteString(let requiredLength):
            guard case .byteString(let data) = value else { return false }
        ...
        case .array(let requiredElements):
        ...
        case .tagged(let requiredTag):
            guard case .tagged(let tag, _) = value else { return false }
...
extension CBORReader {
    ...
    func validateTagWithIANA(tag: UInt64, value: CBORValue) throws {
        ...
        if !definition.dataItem.validate(value) {
            throw CBORError.invalidFormat("Tag \(tag) (\(definition.name)) requires \(definition.dataItem), got \(value)")
        }

        // Additional semantic validation for specific tags
        switch tag {
        case 0: // RFC3339 date string
            guard case .textString(let str) = value else { return }
        ...
        case 37: // UUID
            guard case .byteString(let data) = value else { return }
            if data.count != 16 {
