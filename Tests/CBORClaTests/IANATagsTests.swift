...
final class IANATagsTests: XCTestCase {
    ...
    func testStandardDateTimeTag() throws {
        ...
        guard case .tagged(0, let box) = decoded,
              case .textString(let decodedDate) = box.value else {
    ...
    func testEpochDateTimeTag() throws {
        ...
        let epochTime: Double = 1726749296.5
        var value = epochTime.bitPattern.bigEndian
        encoded.append(contentsOf: withUnsafeBytes(of: &value, Array.init))

        let decoded = try decoder.decodeValue(from: encoded)
        guard case .tagged(1, let box) = decoded,
              case .float64(let decodedTime) = box.value else {
    ...
    func testUUIDTag() throws {
        ...
        let decoded = try decoder.decodeValue(from: encoded)
        guard case .tagged(37, let box) = decoded,
              case .byteString(let decodedData) = box.value else {
    ...
    func testLanguageTaggedString() throws {
        ...
        let decoded = try decoder.decodeValue(from: encoded)
        guard case .tagged(38, let box) = decoded,
              case .array(let arr) = box.value,
              arr.count == 2,
              case .textString(let decodedLang) = arr[0],
              case .textString(let decodedText) = arr[1] else {
    ...
    func testIPv4Tag() throws {
        ...
        let decoded = try decoder.decodeValue(from: encoded)
        guard case .tagged(260, let box) = decoded,
              case .byteString(let decodedIP) = box.value else {
    ...
    func testTypedArrayTags() throws {
        ...
        let decoded = try decoder.decodeValue(from: encoded)
        guard case .tagged(64, let box) = decoded,
              case .byteString(let decodedData) = box.value else {
    ...
    func testDaysSinceEpochTag() throws {
        ...
        let decoded = try decoder.decodeValue(from: encoded)
        guard case .tagged(100, let box) = decoded,
              case .unsigned(let decodedDays) = box.value else {
    ...
    func testSelfDescribedCBOR() throws {
        ...
        let decoded = try decoder.decodeValue(from: encoded)
        guard case .tagged(55799, let box) = decoded,
              case .textString(let decodedText) = box.value else {
